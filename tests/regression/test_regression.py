...
GOLDEN_TESTS = [
    ...
    },
    {
        ...
        "expected_contains": ["async def fetch():", "    async with session:", "        async for item in items:", "            print(item)"]
    },
    {
        "name": "python_match",
        "input": "match x:\ncase 1:\nprint('one')\ncase _:\nprint('other')",
        "expected_contains": ["match x:", "    case 1:", "        print('one')", "    case _:", "        print('other')"]
    }
]
...
def test_golden_output(test_case):
    ...
    for expected in test_case["expected_contains"]:
        assert expected in result.content, f"Expected '{expected}' not found in output:\n{result.content}"
