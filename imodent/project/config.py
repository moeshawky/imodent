...
def load_config_from_yaml(path: Path) -> dict | None:
    ...
    try:
        ...
        print(
            f"Warning: {path.name} exists but could not be parsed: {e}. Using defaults.",
            file=sys.stderr,
        )
    return None
...
def load_config_from_pyproject(path: Path) -> dict | None:
    ...
    try:
        ...
        print(
            f"Warning: pyproject.toml exists but could not be parsed: {e}. Using defaults.",
            file=sys.stderr,
        )
        return None
...
def _apply_dict_to_config(config: AnalysisConfig, data: dict) -> AnalysisConfig:
    ...
    if "check_imports" in data:
        config.check_imports = _coerce_bool(
            data["check_imports"], "check_imports", config.check_imports
        )
    if "check_syntax" in data:
        config.check_syntax = _coerce_bool(
            data["check_syntax"], "check_syntax", config.check_syntax
        )
    if "check_lint" in data:
        config.check_lint = _coerce_bool(
            data["check_lint"], "check_lint", config.check_lint
        )
    if "check_types" in data:
        config.check_types = _coerce_bool(
            data["check_types"], "check_types", config.check_types
        )
    if "auto_fix_safe" in data:
        config.auto_fix_safe = _coerce_bool(
            data["auto_fix_safe"], "auto_fix_safe", config.auto_fix_safe
        )
    if "auto_fix_all" in data:
        config.auto_fix_all = _coerce_bool(
            data["auto_fix_all"], "auto_fix_all", config.auto_fix_all
        )
    if "interactive" in data:
        config.interactive = _coerce_bool(
            data["interactive"], "interactive", config.interactive
        )
    ...
    if "use_pyright" in data:
        config.use_pyright = _coerce_bool(
            data["use_pyright"], "use_pyright", config.use_pyright
        )
    if "check_rust" in data:
        config.check_rust = _coerce_bool(
            data["check_rust"], "check_rust", config.check_rust
        )
    if "run_cargo" in data:
        config.run_cargo = _coerce_bool(
            data["run_cargo"], "run_cargo", config.run_cargo
        )
    if "run_cargo_check" in data:
        config.run_cargo_check = _coerce_bool(
            data["run_cargo_check"], "run_cargo_check", config.run_cargo_check
        )
    if "run_cargo_clippy" in data:
        config.run_cargo_clippy = _coerce_bool(
            data["run_cargo_clippy"], "run_cargo_clippy", config.run_cargo_clippy
        )
    return config
