...
class AnalysisCoordinator:
    ...
    def fix(
        ...
        for file_path, file_findings in by_file.items():
            ...
            for finding in sorted(file_findings, key=lambda f: f.location.line if f.location else 0, reverse=True):
                ...
                if mode == FixMode.SAFE_AUTO:
                    if not finding.auto_fix_safe:
                        continue  # Skip
                    option = fixer.get_options(finding, context)[0]  # Use first (safest)
    ...
    def _get_user_choice(self, finding: Finding, fixer, context: AnalysisContext) -> Optional[FixOption]:
